...
CREATE TABLE bc_memb (
    ...
    athcd VARCHAR(255) NULL     COMMENT '인증코드', -- 인증코드
...
CREATE TABLE bc_amount (
    ...
    mno    INTEGER      NOT NULL COMMENT '회원번호', -- 회원번호
    attype VARCHAR(50)  NOT NULL COMMENT '입출금유형', -- 입출금유형
    ...
    hdt    DATE         NOT NULL COMMENT '일시', -- 일시
...
ALTER TABLE bc_amount
    ADD CONSTRAINT PK_bc_amount -- 수입/지출 기본키
        PRIMARY KEY (
            amntno -- 입출금번호
        );
...
-- 수입/지출
ALTER TABLE bc_amount
    ADD CONSTRAINT FK_bc_memb_TO_bc_amount -- 회원 -> 수입/지출
        FOREIGN KEY (
            mno -- 회원번호
        )
...
-- 출금내용
ALTER TABLE bc_contents
    ADD CONSTRAINT FK_bc_amount_TO_bc_contents -- 수입/지출 -> 출금내용
        FOREIGN KEY (
            amntno -- 입출금번호
        )
        REFERENCES bc_amount ( -- 수입/지출
            amntno -- 입출금번호
        );